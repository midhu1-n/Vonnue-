...
### SQLite-Based Persistent Architecture
Although early design discussions explored the possibility of a stateless architecture, the final implementation adopts a persistent data model using SQLite and Django ORM. Decisions, options, criteria, and evaluation scores are stored in a relational database, allowing users to progress through the decision-making workflow while maintaining data consistency across pages and requests.

Using SQLite simplified development, reduced deployment complexity, and provided reliable persistence for the application's core entities. The database integrates seamlessly with Django's model layer, enabling efficient CRUD operations through Django REST Framework APIs. This architecture ensures that decision data, criteria configurations, and evaluation results can be retrieved and reused throughout the decision-analysis process while maintaining a clean separation between the frontend and backend layers.
...
1. **Adopting a Persistent Data Model (SQLite):** During development, I decided to maintain a persistent architecture using SQLite and Django ORM rather than moving to a fully stateless design. Since the application manages decisions, options, criteria, and evaluation scores across multiple workflow stages, persistent storage improved reliability, simplified data retrieval, and enabled seamless communication between the frontend and backend through REST APIs. SQLite provided a lightweight yet effective solution for the project's scope while keeping the architecture simple and maintainable.
